...
describes what users get. Pick a format with `-o`, or let ivivu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ivivu <command> -o table   # aligned columns for reading
ivivu <command> -o jsonl   # one JSON object per line, for piping
ivivu <command> -o json    # a single JSON array
ivivu <command> -o csv     # spreadsheet friendly
ivivu <command> -o tsv     # tab-separated
ivivu <command> -o url     # just the URL column
ivivu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ivivu <command> --fields id,title,url
```
...
```bash
ivivu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ivivu <command>            # a table, because this is a terminal
ivivu <command> | wc -l    # JSONL, because this is a pipe
```
